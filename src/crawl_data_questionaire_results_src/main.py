...
def open_connection(sql_name_in, sql_pw_in):
    # import variables
    if sql_name_in is not None: sql_name = sql_name_in
    else:                       sql_name = env("SQL_DB_NAME")
    if sql_pw_in is not None:   sql_pw = sql_pw_in
...
def main(sql_name_in=None, sql_pw_in=None):
    conn, cursor = open_connection(sql_name_in, sql_pw_in)
    ...
    print("This HTTP triggered function did crawl the required sites and wrote the resultst into an Azuer SQL DB.")
